...
export default function MonthTable({ ELW_monthTable, fontSize, ELW_CallPutRatio_Maturity }) {
    ...
    return (
        <Grid container spacing={1} >
            <Grid item xs={12}>
                {
                    ELW_monthTable.status === 'succeeded' ?
                        <>
                            <Table size="small" sx={{ color: '#efe9e9ed', borderCollapse: 'collapse' }}>
                                <TableHead>
                                    ...
                                    <TableRow>
                                        ...
                                        {ELW_monthTable.data.slice(-3).map((item, index) => (
                                ...
                                <TableBody>
                                    {['WA2', 'WA3', 'WA4', 'WA1'].map((wa, index) => {
                                        const 차이1 = ELW_monthTable.data[3][wa] - ELW_monthTable.data[0][wa]
                                        const 차이2 = ELW_monthTable.data[4][wa] - ELW_monthTable.data[1][wa]
                                        const 차이3 = ELW_monthTable.data[5][wa] - ELW_monthTable.data[2][wa]
                                        ...
                                        return <TableRow key={index}>
                                            ...
                                            <TableCell sx={tableBodyStyle} align='center'>{ELW_monthTable.data[3][wa].toFixed(2)}</TableCell>
                                            <TableCell sx={{ ...tableBodyStyle2, color: color1 }} align='center'>{차이1.toFixed(2)}</TableCell>
                                            <TableCell sx={tableBodyStyle} align='center'>{ELW_monthTable.data[4][wa].toFixed(2)}</TableCell>
                                            <TableCell sx={{ ...tableBodyStyle2, color: color2 }} align='center'>{차이2.toFixed(2)}</TableCell>
                                            <TableCell sx={tableBodyStyle} align='center'>{ELW_monthTable.data[5][wa].toFixed(2)}</TableCell>
                                    ...
                                    <TableRow>
                                        ...
                                        {['3', '4', '5'].map(idx => {
                                            const avg = (ELW_monthTable.data[idx]['WA2'] + ELW_monthTable.data[idx]['WA3'] + ELW_monthTable.data[idx]['WA4'] + ELW_monthTable.data[idx]['WA1']) / 4;
                                            const diffAvg = (ELW_monthTable.data[idx]['WA2'] - ELW_monthTable.data[idx - 3]['WA2'] +
                                                ELW_monthTable.data[idx]['WA3'] - ELW_monthTable.data[idx - 3]['WA3'] +
                                                ELW_monthTable.data[idx]['WA4'] - ELW_monthTable.data[idx - 3]['WA4'] +
                                                ELW_monthTable.data[idx]['WA1'] - ELW_monthTable.data[idx - 3]['WA1']) / 4;
                                            return (
                                                <>
