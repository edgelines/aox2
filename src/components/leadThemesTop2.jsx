...
export default function LeadThemesTopPage({ swiperRef }) {
    ...
    useEffect(() => {
        ...
        ws.onopen = () => {
            console.log('Lead ThemesTop2 WebSocket Connected');
        };
        ...
        ws.onerror = (error) => {
            console.error('Lead ThemesTop2 WebSocket Error: ', error);
        };

        ws.onclose = () => {
            console.log('Lead ThemesTop2 WebSocket Disconnected');
        };
    ...
    return (
        <Grid container >
            ...
            <Box sx={{ position: 'absolute', transform: 'translate(1250px, 540px)', zIndex: 0, backgroundColor: 'rgba(0, 0, 0, 0.2)', fontSize: '14px', textAlign: 'left' }}>
                <TableContainer >
                    <Table size='small'>
                        <TableHead>
                            <tr style={{ fontSize: '11px' }}>
