...
import { WEBSOCKET_CONNECT_WA_1, WEBSOCKET_DISCONNECT_WA_1, websocketMessageWA1 } from '../actions/websocketActions';
...
function createWebSocketChannel(socket) {
    return eventChannel(emit => {
        socket.onmessage = event => {
            ...
            emit(websocketMessageWA1(JSON.parse(month1)));
        };
...
function* handleWebSocketConnection() {
    const socket = new WebSocket(`${API_WS}/elwMonth1`);
...
export default function* WA_1() {
    while (yield take(WEBSOCKET_CONNECT_WA_1)) {
        const websocketTask = yield fork(handleWebSocketConnection);
        yield take(WEBSOCKET_DISCONNECT_WA_1);
